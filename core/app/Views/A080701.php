      ...
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-12">
            ...
            <h1>
              <?php echo $NamaMenu ?>
              <i class="fas fa-question-circle" data-toggle="modal" data-target="#keteranganModal" style="font-size: 16px; cursor: pointer; position: relative; top: -4px; "></i>
            </h1>
          </div>
              ...
              if (isset($oQueryList)) {
                if (count($oQueryList) > 0) {
                  foreach ($oQueryList as $oRS) {
                    ...
                    $pos = substr(rand(), 0, 1);
                    $mnu_dtl = $pos . substr($hash, 0, 128 - intval($pos)) . $AppClass->KodeMenu . substr($hash, -1 * intval($pos));
                    $mnu_dtl .= "/dtl/per" . $oRS->Periode . "/cod" . $oRS->idx . "/";
                    $hash = hash('sha512', rand());
                    $pos = substr(rand(), 0, 1);
                    $mnu_frm = $pos . substr($hash, 0, 128 - intval($pos)) . "080702" . substr($hash, -1 * intval($pos));
                    $mnu_frm .= "/frm/per" . $oRS->Periode . "/cod" . $oRS->idx . "/";
              ?>
                    <tr>
                      ...
                      <?php if(is_null($oRS->LaksanaOlehNama) || $oRS->KodePosisiUser==$oRS->LaksanaOleh) { ?>
                        <div class="btn-group" role="group" aria-label="Action Button">
                          <a href="<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu_frm ?>" class="btn btn-xs btn-info control-edit" title="Buka Form Tindak Lanjut Disposisi"><i class="fas fa-edit small text-white"></i></a>
                        </div>
                      <?php } else { ?>
                        <div class="btn-group" role="group" aria-label="Action Button">
                          <a href="<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu_dtl ?>" class="btn btn-xs btn-info control-view" title="Lihat Detail Tindak Lanjut Disposisi"><i class="fas fa-eye small text-white"></i></a>
                        </div>
                      <?php }  ?>
                      </td>
                    </tr><?php

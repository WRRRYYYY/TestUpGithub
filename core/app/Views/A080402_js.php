    ...
    <script type="text/javascript">
    	$(document).ready(function() {
			...
			$mnu2 = $pos . substr($hash, 0, 128 - intval($pos)) . "081403" . substr($hash, -1 * intval($pos));
			$mnu3 = $pos . substr($hash, 0, 128 - intval($pos)) . "081405" . substr($hash, -1 * intval($pos));
			?>
    		var url_inbox = '<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu1; ?>';
    		var url_verif = '<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu2 . "/frm/"; ?>';
    		var url_asman = '<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu3 . "/frm/"; ?>';
    		...
    		$('#KodeHeader').on('change', function() {
    			var selectedOption = $("#KodeHeader option:selected");
    			var selectedValue = $(this).val();
    			var isi = selectedOption.data('isi');

    			if (selectedValue !== "") {
    				$('#isiKopContent').html(isi);
    				$('#isiKopContainer').removeClass('d-none');
    			} else {
    				$('#isiKopContainer').addClass('d-none');
    				$('#isiKopContent').html('');
    			}
    		});
    		$(function() {
    			var input = document.querySelector('#Tembusan');
    			new Tagify(input, {
    				delimiters: ",", // tekan koma jadi tag
    				dropdown: {
    					enabled: 0 // tidak muncul dropdown otomatis
    				}
    				// ,
    				// //tagTextProp: "value", // bisa tentukan properti tag yang digunakan
    				// style: {
    				// 	fontSize: "16px", // Mengatur ukuran font secara langsung di Tagify
    				// 	padding: "1000px 10px"
    				// }
    			});
    		});
    		...
    		$('.inline-editor').each(function() {
    			const editor = new Jodit(this, {
    				...
    				buttons: [
    					'source',
    					'paragraph', 'fontsize', 'bold', 'italic', 'underline', 'strikethrough',
    					'brush', 'align', 'ul', 'ol', 'table', 'link', 'lineHeight', 'image', 'undo', 'redo'
    				],
    				uploader: {
    			...
    			// Simpan instance ke data DOM
    			$(this).data('joditInstance', editor);
    		});


    		$('.inline-editor_').summernote({
    			//				dialogsInBody: true,
    			height: 150,
    												...
    												if (arResp[2].indexOf("<!--##-->") > 0) {
    													...
    													setTimeout(function() {
    														...
    														if ($('#image').cropper('getData').height > 0) {
    															saveDataFoto(arResp[1], UploadNeeded, RemoveNeeded);
    														} else if (UploadNeeded) {
    															uploadFile();
    															...
    															//	Swal.close();
    															// Swal.fire({
    															// 	title: "Pilih tindakan untuk surat ini?",
    															// 	showCancelButton: true,
    															// 	showDenyButton: true,
    															// 	confirmButtonText: "Ajukan Verifikasi",
    															// 	cancelButtonText: "Tutup",
    															// 	denyButtonText: "Ajukan Asman",
    															// 	customClass: {
    															// 		confirmButton: "btn btn-primary swal-button",
    															// 		cancelButton: "btn btn-secondary swal-button",
    															// 		denyButton: "btn btn-danger swal-button"
    															// 	}
    															// }).then((result) => {
    															// 	let kodeLama = $("#page_form .form-control[name='KodeLama']").val(); // Ambil nilai KodeLama

    															// 	if (result.isConfirmed) {
    															// 		pleaseWait();
    															// 		window.location = url_verif + "cod" + kodeLama;
    															// 	} else if (result.isDenied) {
    															// 		pleaseWait();
    															// 		window.location = url_asman + "cod" + kodeLama;
    															...
    															//Swal.close();
    															let roleAlert = $("#page_form .form-control[name='RoleAlert']").val(); // Ambil RoleAlert
    															let kodeLama = $("#page_form .form-control[name='KodeLama']").val(); // Ambil KodeLama

    															// Siapkan konfigurasi awal
    															let showConfirm = false;
    															let showDeny = false;

    															// Atur tombol berdasarkan role
    															if (roleAlert === "katu" || roleAlert === "sekretaris") {
    																showConfirm = true;
    																showDeny = true;
    															} else if (roleAlert === "tu") {
    																showConfirm = false;
    																showDeny = true;
    															} else {
    																// Role lain, default semua tombol tampil
    																showConfirm = false;
    																showDeny = false;
    															}

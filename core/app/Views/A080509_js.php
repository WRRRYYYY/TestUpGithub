...
//			$(".control-delete").on("click", function(e) {
			$("#datatable").on("click","a.control-undo", function(e) { 
				e.preventDefault();
				var btn = $(this);
				Swal.fire({
					  title: "<span style='color:#dc3545'>Anda yakin?</span>",
					  text: "Anda akan membatalkan disposisi surat ini",
					  icon: "question",
...
//									window.location = page_list;
									Swal.fire({ title: "Success", text: "Data berhasil dibatalkan",icon: "success",})
										.then((result) => {
											if (result.isConfirmed) { 
												pleaseWait();
												window.location.reload();

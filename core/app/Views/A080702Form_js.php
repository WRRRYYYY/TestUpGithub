    ...
    <script type="text/javascript">
    	$(document).ready(function() {
    		...
    		var RemoveStatus;
    		var jsonPreloaded = [];
    		var FolderDoc = "dispo_laksana";
    		<?php
			$hash = hash('sha512', rand());
			$pos = substr(rand(), 0, 1);
			$mnu = $pos . substr($hash, 0, 128 - intval($pos)) . "080701" . substr($hash, -1 * intval($pos));
			?>
    			...
    			$("#page_form .fileinputt").each(function() {
    				if ($(this).find(".form-control[name^='FileOri']").val() != "" &&
    					...
    					var param = 'href=' + escape(page_href) + "&",
    						paramdtls = '';
    					param += "&Kode=" + escape($("#page_form .form-control[name='KodeLama']").val()+"###"+$("#page_form .form-control[name='Seq']").val());
    		...
    		function uploadFile() {
    			...
    			$("#page_form .fileinputt").each(function() {
    				if ($(this).find(".file-input")[0].files.length !== 0) {
    					...
    					formData.append('Kode', $("#page_form .form-control[name='KodeLama']").val()+"###"+$("#page_form .form-control[name='Seq']").val());

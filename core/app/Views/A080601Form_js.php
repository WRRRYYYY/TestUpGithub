    ...
    <script type="text/javascript">
    	$(document).ready(function() {
    		...
    		$("#AgendaPimpinan").change(function() {
    		...
    		if ($("#AgendaPimpinan").prop("checked")) {
    			$("#agendaGroup").show();
    		}
    		...
    		$('#Pengolah').select2({
    			placeholder: "Pilih Instansi Pengolah"
    		}).on('change', function() {}).trigger('change');
    		...
    		$('#CatatanArsip').select2({
    			placeholder: "Pilih Catatan Arsip"
    		}).on('change', function() {}).trigger('change');

			$('#AjukanKe').select2({
    			placeholder: "Pilih Ajukan Ke"
    		}).on('change', function() {}).trigger('change');

			$('#AjukanCC').select2({
    			placeholder: "Pilih Ajukan CC"
    		}).on('change', function() {}).trigger('change');

			$('#AjukanCaption').select2({
    			placeholder: "Pilih Ajukan Caption"
    		}).on('change', function() {}).trigger('change');
    	...
    	$('#AjukanKe').on('change', function () {
  var isValid = false;

  $('#AjukanKe option:selected').each(function () {
...
$('#AjukanKe').trigger('change');

    	
    	document.getElementById("btnSubmit").addEventListener("click", async function() {
    		const selectAjukanKe = document.getElementById("AjukanKe");
    		const selectAjukanCaption = document.getElementById("AjukanCaption");
    		const textareaCatatan = document.getElementById("AjukanCatatan");
    		const checkboxNotif = document.getElementById("kirimNotif");

    		const selectedOption = selectAjukanKe.options[selectAjukanKe.selectedIndex];
    		const noPegawai = selectedOption?.getAttribute("data-nopegawai");

    		if (!noPegawai) {
    			alert("Silakan pilih pejabat penerima terlebih dahulu.");
    			return;
    		}

    		const captions = Array.from(selectAjukanCaption.selectedOptions).map(opt => opt.text).join(", ");
    		const catatan = textareaCatatan.value.trim();

    		// Ambil tanggal surat dari <span> yang mengandung label "Tanggal Surat"
    		let tanggalSurat = "-";
    		let perihal = "-";
    		...
    		const linkSurat = "https://baznasjateng.eoffice.web.id";
...
👉 Klik di sini untuk membuka surat: ${linkSurat}`;
    		...
    		if (checkboxNotif && checkboxNotif.checked) {
    			...
    			try {
    				...
    				console.log(res);
    				alert("Notifikasi berhasil dikirim ke " + noPegawai);

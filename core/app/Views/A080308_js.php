	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$mnu2 = $pos . substr($hash, 0, 128 - intval($pos)) . "080301" . substr($hash, -1 * intval($pos));
			?>
			var page_list = '<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu; ?>';
			var page_form = '<?php echo  base_url() . "/" . strtolower($controller_name) . "/mnu/" . substr(md5(rand()), 4, 7) . "/salt/" . $mnu2; ?>';
...
// 			});
	    	...
	    	$(document).on('click', '.btn-preview', function() {
	   			var docUrl = $(this).data('url'); // Ambil URL dokumen
	   			var downloadUrl = $(this).data('download'); // Ambil URL download
				...
				$('#iframeDokumen').attr('src', downloadUrl); // Set iframe
				$('#btnDownload').attr('href', downloadUrl); // Set tombol Download
				$('#modalDokumen').modal('show'); // Buka modal
			});
